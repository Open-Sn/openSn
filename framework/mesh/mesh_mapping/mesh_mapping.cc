...

{
  ...
  if (fine_grid.Dimension() != coarse_grid.Dimension())
    OpenSnLogicalError("Grid dimensions are not equal for mapping. Fine dimension = " +
                       std::to_string(fine_grid.Dimension()) +
                       ", coarse dimension = " + std::to_string(coarse_grid.Dimension()) + ".");

  coarse_to_fine_.clear();
  fine_to_coarse_.clear();

  // Instantiate the maps; *Mapping constructor takes the cell to size the face maps.
  ...
  for (const auto& [coarse_cell_ptr, coarse_mapping] : coarse_to_fine_)
  {
    ...
    const auto coarse_cell_volume = coarse_cell_mapping.CellVolume();
    ...
    for (const auto fine_cell_ptr : coarse_mapping.fine_cells)
    {
      ...
      total_fine_volume += fine_cell_mapping.CellVolume();
    }
